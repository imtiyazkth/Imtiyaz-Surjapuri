...
export default function ArticlePage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/articles?limit=100")
      ...
      .then((d) => {
        ...
        // Multi-strategy matching — handles all slug formats
        const found =
          list.find((a) => a.slug === slugParam) ??
          list.find((a) => a.id  === slugParam) ??
          list.find((a) => a.slug?.replace(/[^a-z0-9]+/g,"-") === slugParam.replace(/[^a-z0-9]+/g,"-")) ??
          list.find((a) => slugParam.includes(a.id)) ??
          list.find((a) => a.slug?.includes(slugParam.split("-").slice(0,-1).join("-")));
  ...
  // ── Fix 5: Like button with optimistic update ────────────────
  ...
  // ── Fix 5: Social share using Web Share API + fallback ───────
  const handleShare = async (platform?: string) => {
    ...
    // Native Web Share API (works great on Android)
    ...
    const shareUrls: Record<string, string> = {
      ...
      twitter:  `https://x.com/intent/tweet?url=${encoded}&text=${encodedTitle}`,
  ...
  if (loading) return (
    <div style={{ maxWidth:"820px", margin:"0 auto", padding:"32px 16px" }}>
      {[220, 40, 30, 160, 100, 140, 100].map((h, i) => (
    ...
    <div style={{ maxWidth:"820px", margin:"0 auto", padding:"28px 16px 60px" }}>
      ...
      <div style={{
        ...
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap", alignItems:"center" }}>
          <span style={{ fontSize:"0.75rem", color:"var(--text-3)", fontFamily:"var(--font-sans)" }}>
            Share:
          </span>
          ...
          {typeof navigator !== "undefined" && "share" in navigator && (
            <button onClick={() => handleShare()}
              style={{
                padding:"7px 14px", borderRadius:"20px", fontSize:"0.78rem",
